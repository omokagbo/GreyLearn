// FeaturePath/Presentation/CustomViews/LearningPathConnector.swift
import SwiftUI
import Domain
import DesignSystem

public struct LearningPathConnector: View {
    let modules: [Module]
    ...
    public init(
        modules: [Module],
        anchors: [Int: Anchor<CGPoint>],
        lineWidth: CGFloat,
        curveOffset: CGFloat,
        curveEdgeInset: CGFloat,
        curveOvershoot: CGFloat,
        curveLonelyBoost: CGFloat
    ) {
        self.modules          = modules
        self.anchors          = anchors
        self.lineWidth        = lineWidth
        self.curveOffset      = curveOffset
        self.curveEdgeInset   = curveEdgeInset
        self.curveOvershoot   = curveOvershoot
        self.curveLonelyBoost = curveLonelyBoost
    }
    ...
    private func resolvePoints(in proxy: GeometryProxy) -> [Int: CGPoint] {
        ...
        for module in modules {
            if let anchor = anchors[module.id] { points[module.id] = proxy[anchor] }
        }
        return points
    }
    ...
    private func buildPaths(points: [Int: CGPoint], containerWidth: CGFloat) -> (solid: Path, dotted: Path) {
        ...
        for index in 0..<(modules.count - 1) {
            ...
            if isSameRow {
                ...
                let isLonelyConnection = !modules.count.isMultiple(of: 2) && (index + 1) == modules.count - 1
                ...
                let control1   = CGPoint(x: controlX, y: start.y)
                let control2   = CGPoint(x: controlX, y: end.y)
